            ...
            <div class="row p-2">
                ...
                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12 col-xs-12 animated fadeIn">
                    <div data-toggle="tooltip" data-placement="bottom" data-title="Total amount of pending fine" class="dashcounter color-two p-4 box-hover">
                        ...
                        ?>                 
                        <p class="dashcount-name">Pending Fine Amount </p>
                    </div>
                </div>
                ...
                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12 col-xs-12 animated fadeIn">
                    <div data-toggle="tooltip" data-placement="bottom" data-title="Total amount of paid fine" class="dashcounter color-four p-4 box-hover">
                        ...
                        echo '<h3 class="counter">'.$total.'</h3>';
                        ?>
                        <p class="dashcount-name">Paid Fine Amount </p>
                    </div>
                </div>
                <!--Third count box end-->
...
<?php
    include "../connection.php";
    $year = date("Y");
    // $yearString = date_format($year,'Y');
    $yearString = intval($year);
    ...
    $jan = mysqli_query($conn, "CALL GetFinesByMonthAndYear(01, '$yearString')");
    if ($jan) {
        $janVal = mysqli_num_rows($jan);
        mysqli_free_result($jan); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    }
    mysqli_next_result($conn);
    $feb = mysqli_query($conn, "CALL GetFinesByMonthAndYear(02, '$yearString')");

if ($feb) {
    $febVal = mysqli_num_rows($feb);
    mysqli_free_result($feb); // Free the result set
    // Process $febVal or perform other operations
} else {
    // Query execution failed
    echo "Error executing query: " . mysqli_error($conn);
...
mysqli_next_result($conn);
    $march = mysqli_query($conn, "CALL GetFinesByMonthAndYear(03, '$yearString')");
    if ($march) {
        $marchVal = mysqli_num_rows($march);
        mysqli_free_result($march); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $april = mysqli_query($conn, "CALL GetFinesByMonthAndYear(04, '$yearString')");
    if ($april) {
        $aprilVal = mysqli_num_rows($april);
        mysqli_free_result($april); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $may = mysqli_query($conn, "CALL GetFinesByMonthAndYear(05, '$yearString')");
    if ($may) {
        $mayVal = mysqli_num_rows($may);
        mysqli_free_result($may); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $june = mysqli_query($conn, "CALL GetFinesByMonthAndYear(06, '$yearString')");
    if ($june) {
        $juneVal = mysqli_num_rows($june);
        mysqli_free_result($june); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $july = mysqli_query($conn, "CALL GetFinesByMonthAndYear(07, '$yearString')");
    if ($july) {
        $julyVal = mysqli_num_rows($july);
        mysqli_free_result($july); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $august = mysqli_query($conn, "CALL GetFinesByMonthAndYear(08, '$yearString')");
    if ($august) {
        $augustVal = mysqli_num_rows($august);
        mysqli_free_result($august); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $september = mysqli_query($conn, "CALL GetFinesByMonthAndYear(09, '$yearString')");
    if ($september) {
        $sepVal = mysqli_num_rows($september);
        mysqli_free_result($september); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $october = mysqli_query($conn, "CALL GetFinesByMonthAndYear(10, '$yearString')");
    if ($october) {
        $octVal = mysqli_num_rows($october);
        mysqli_free_result($october); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $november = mysqli_query($conn, "CALL GetFinesByMonthAndYear(11, '$yearString')");
    if ($november) {
        $novVal = mysqli_num_rows($november);
        mysqli_free_result($november); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
    ...
    mysqli_next_result($conn);
    $december = mysqli_query($conn, "CALL GetFinesByMonthAndYear(12, '$yearString')");
    if ($december) {
        $decVal = mysqli_num_rows($december);
        mysqli_free_result($december); // Free the result set
        // Process $febVal or perform other operations
    } else {
        // Query execution failed
        echo "Error executing query: " . mysqli_error($conn);
